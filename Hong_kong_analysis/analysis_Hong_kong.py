...
class MetroNetworkAnalyzer:
    ...
    def __init__(self, geojson_path, cost_per_km=50000000):
        ...
        self.stations = {}
        self.line_colors = {}
        self.line_orders = {}       # line -> ordered list of station_ids
        self.broken_segments = set()  # set of (u,v) tuples representing permanently broken track segments
    ...
    def _build_edges(self):
        """Build edges between consecutive stations on same line and store canonical order."""
        # Group stations by line
        line_stations = {}
        for station_id, data in self.stations.items():
            for line in data['lines']:
                line_stations.setdefault(line, []).append(station_id)

        # Connect stations on same line based on geographical proximity
        for line, stations in line_stations.items():
            if len(stations) < 2:
                self.line_orders[line] = stations
                continue
            ...
            # Sort by coordinates to establish order (consistent canonical ordering)
            sorted_indices = np.lexsort((coords[:, 1], coords[:, 0]))
            sorted_stations = [station_ids[i] for i in sorted_indices]
            # Save canonical order
            self.line_orders[line] = sorted_stations

            # Connect consecutive stations only (store an attribute to mark original line edges)
            for i in range(len(sorted_stations) - 1):
                ...
                # mark original physical edge
                self.G.add_edge(s1, s2, weight=dist, line=line, original_line_edge=True)

        # Also connect interchange stations (same name, different coordinates)
        station_names = {}
        for station_id, data in self.stations.items():
            name = data['name']
            station_names.setdefault(name, []).append(station_id)

        for name, station_list in station_names.items():
            if len(station_list) > 1:
                # Connect all pairs of same-named stations (interchange links)
                for i in range(len(station_list)):
                    for j in range(i + 1, len(station_list)):
                        a, b = station_list[i], station_list[j]
                        if not self.G.has_edge(a, b):
                            self.G.add_edge(a, b, weight=0.1, line='interchange', original_line_edge=False)

    
    def remove_node_and_break(self, node_id):
        """
        Remove a node and record broken track segments so neighbors are NOT bridged.
        This will:
        - remove node_id from the graph
        - mark station as removed (self.stations[node_id]['removed'] = True)
        - for each line the station belonged to, if it had prev and next neighbors,
            record (prev, next) and (next, prev) in self.broken_segments
        """
        if node_id not in self.G:
            # it's already removed from graph — still mark as removed
            if node_id in self.stations:
                self.stations[node_id]['removed'] = True
            return

        # find neighbors according to canonical line order(s)
        lines = self.stations[node_id].get('lines', [])
        for line in lines:
            order = self.line_orders.get(line, None)
            if order is None:
                continue
            if node_id not in order:
                continue
            idx = order.index(node_id)
            prev_node = order[idx - 1] if idx - 1 >= 0 else None
            next_node = order[idx + 1] if idx + 1 < len(order) else None

            # If both prev and next exist, record that the segment between them is broken
            if prev_node and next_node:
                self.broken_segments.add((prev_node, next_node))
                self.broken_segments.add((next_node, prev_node))

        # mark station removed metadata (so plotting/logic can use it)
        if node_id in self.stations:
            self.stations[node_id]['removed'] = True

        # finally remove the node from graph (this removes its incident edges)
    ...
    def plot_network(self, criticality=None, title="Metro Network", 
        ...
        for u, v, data in self.G.edges(data=True):
            ...
            # if there exists a broken segment that would connect u and v, skip drawing this edge
            if (u, v) in getattr(self, 'broken_segments', set()):
                continue
        ...
        # --- guard add_resilience_lines so it doesn't reconnect broken segments ---
        # inside the loop where you would add edge between kth_critical_node and nearest_node:
    ...
    def attack_critical_nodes(self, top_k=3, color_code_lines=False):
        ...
        for i in range(min(top_k, len(sorted_nodes))):
            ...
            # Remove node
            self.remove_node_and_break(node)
        ...
        for line, nodes in line_nodes.items():
            ...
            for target_line in most_critical_lines:
                ...
                if target_line not in line_nodes:
                    continue
                if (kth_critical_node, nearest_node) in self.broken_segments or (nearest_node, kth_critical_node) in self.broken_segments:
                 # skip adding this resilience edge because it would re-join a broken original segment
                    continue
...
def main():
    ...
    geojson_file = "delhi_metro_stations.geojson"  # Replace with your file
